...
def cal_train_metrics(args, msg: dict, outs: dict, labels: torch.Tensor, batch_size: int):
    ...
    if args.use_fpn:
        for i in range(1, 5):
            acc = top_k_corrects(
                outs["layer"+str(i)].mean(1), labels, tops=[1])["top-1"] / batch_size
            acc = round(acc * 100, 2)
    ...
    if args.use_combiner:
        acc = top_k_corrects(outs['comb_outs'], labels, tops=[1])[
            "top-1"] / batch_size
        acc = round(acc * 100, 2)
    ...
    if "ori_out" in outs:
        acc = top_k_corrects(outs["ori_out"], labels, tops=[1])[
            "top-1"] / batch_size
        acc = round(acc * 100, 2)
...
def _cal_evalute_metric(corrects: dict,
    ...
    tmp_corrects = top_k_corrects(tmp_score, labels, tops=[1, 3])
        ...
        for batch_id, (ids, datas, labels) in enumerate(test_loader):
            ...
            if args.use_fpn:
                for i in range(1, 5):
                    ...
                    _cal_evalute_metric(corrects, total_samples, outs[this_name].mean(
                        1), labels, this_name, scores, score_names)
            ...
            if args.use_selection:
                for name in outs:
                    ...
                    labels_1 = labels.unsqueeze(1).repeat(1, S).flatten(0)
                    _cal_evalute_metric(
                        corrects, total_samples, logit, labels_1, this_name)
                ...
                for name in outs:
                    ...
                    labels_0 = labels.unsqueeze(1).repeat(1, S).flatten(0)
                    _cal_evalute_metric(
                        corrects, total_samples, logit, labels_0, this_name)

            if args.use_combiner:
                this_name = "combiner"
                _cal_evalute_metric(
                    corrects, total_samples, outs["comb_outs"], labels, this_name, scores, score_names)

            if "ori_out" in outs:
                this_name = "original"
                _cal_evalute_metric(corrects, total_samples,
                                    outs["ori_out"], labels, this_name)
...
def evaluate_cm(args, model, test_loader):
    ...
    with torch.no_grad():
        ...
        df = pd.DataFrame(results, columns=[
                          "id", "original_label", "predict_label", "goal"])
        ...
        plot_confusion_matrix(
            df_confusion, folders, args.save_dir + "infer_cm.png", accuracy=best_top1)
...
def get_pres_name(img_name, pill2pres):
    print(img_name)
    for k, v in pill2pres.items():
        if img_name in v:
            return k + ".png"
    return ""
...
#        print("text", text)
        ...
        if not cls_ids:
            continue
        print("text", text, "cls_ids", cls_ids)
...
@torch.no_grad()
def my_evaluate_cm(args, model, test_loader):
    """
    [Notice: Costom Model]
    If you use costom model, please change fpn module return name (under
    if args.use_fpn: ...)
    ...
    total_samples = {}
    results = []
    path_to_mapping_name_cls = args.path_map_name2cls
    name2cls = json.load(open(path_to_mapping_name_cls))

    path_to_ocr = Path(args.ocr)
    ocr_results = json.load(path_to_ocr.open("r", encoding="utf-8"))

    path_pill2pres = Path(args.pill_pres_map)
    pill2pres = json.load(path_pill2pres.open("r"))
    ...
    if args.use_mapping:
        mapping_cls = json.load(open("map_cls.json"))
    else:
        mapping_cls = {str(i): i for i in range(107)}

    with torch.no_grad():
        """ accumulate """
        for batch_id, (ids, datas, labels) in tqdm(enumerate(test_loader)):
            ...
            for i in range(scores[0].shape[0]):
                name_img = test_loader.dataset.data_infos[ids[i].item()]['path'].split(
                    "/")[-1]
                name_img = name_img.split("$")
                last_img = name_img[-1]
                # print(last_img)
                row = int(name_img[0]) + 1
                pres_ocr = ocr_results[get_pres_name(last_img, pill2pres)]
                pres_pills = process_pres(pres_ocr, name2cls)
                # print(clsId , pres_pills)
                topk = torch.topk(scores[0][i], 3)

                topk_prob = topk.values.tolist()
                topk_idx = topk.indices.tolist()

                # clsId = mapping_cls[str(scores[0][i].argmax().item())]
                # prob = scores[0][i][scores[0][i].argmax().item()].item()
                # clsId,prob = custom_post_processv1(clsId,prob,scores)

                topk_idx = [str(mapping_cls[str(i)]) for i in topk_idx]
                topk_prob = [str(i) for i in topk_prob]
                # clsId = 107
                # prob = 1
                # for idx in topk_idx:
                #     if idx in pres_pills:
                #         clsId = idx
                #         prob = scores[0][i][scores[0]
                #                             [i].argmax().item()].item()
                #         break

                # if prob <= 0.52 or clsId not in pres_pills:
                #     clsId = 107
                #     prob = 1
                # print(lines)
                lines[row][1] = ",".join(topk_idx)
                lines[row][2] = ",".join(topk_prob)
...
def plot_confusion_matrix(cm, label_names, save_name, title='Confusion Matrix acc = ', accuracy=0):
    ...
    for x_val, y_val in zip(x.flatten(), y.flatten()):
        try:
            ...
            c = 0
        if c > 0.001:
            plt.text(x_val, y_val, "%0.1f" % (c,), color='red',
