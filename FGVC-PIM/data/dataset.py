...
def build_loader(args):
    ...
    if args.train_root is not None:
        train_set = ImageDataset(
            istrain=True, root=args.train_root, data_size=args.data_size, return_index=True)
    ...
    if args.val_root is not None:
        val_set = ImageDataset(
            istrain=False, root=args.val_root, data_size=args.data_size, return_index=True)
        val_loader = torch.utils.data.DataLoader(
            val_set, num_workers=1, shuffle=True, batch_size=args.batch_size)
...
class ImageDataset(torch.utils.data.Dataset):
        ...
        if istrain:
            ...
            self.transforms = transforms.Compose([
                ...
                transforms.RandomApply([transforms.GaussianBlur(
                    kernel_size=(5, 5), sigma=(0.1, 5))], p=0.3),
                transforms.RandomAdjustSharpness(
                    sharpness_factor=1.5, p=0.1),
                transforms.ToTensor(),
                normalize
            ])
        else:
    ...
    def getDataInfo(self, root):
        ...
        folders = sorted([int(p) for p in os.listdir(root)])  # Sort by number
